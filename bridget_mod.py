...
class BRIDGET:
    def __init__(self, user_name, df_batch1, df_batch3, batch1_test, target, user_model, 
                 protected, cats, num,
                 drift_threshold,  
                 belief_threshold, 
                 tau_threshold,
        ...
        # inserito all'inizio perchè funge da "lookup" dict da quale attingere la riga giusta da appendere al log con i valori delle nuove colonne nella fase MiC
        ...
        # self. X è una lista di dizionari, è comodo per costruire il logging, quindi questo va fatto pure per il df batch3 separatamente
        ...
        self.Y = [int(y) for y in self.Y] # trasformazione dei target in valori int
        # può dare problemi nella fase MiC perchè la deferral Net accetta solo tensori
        
        self.X_test1= batch1_test.loc[:, batch1_test.columns != target]  ### porzione test del batch 1
        ...
        self.target = target # nome della colonna
        self.attr_list = list(df_batch1.columns) # nomi delle colonne
        self.protected= protected  # deve essere una lista di stringhe
        ...
        self.user_model = user_model #assunto già fittato
        
        # vari thresholds
        self.drift_threshold = drift_threshold # alpha
    ...
    def switch_phase(self, drift_detected, current_phase, current_log):
        ...
        if drift_detected:
            ...
            if current_phase == 'HiC':
                print(f"Current alpha drift threshold: {self.drift_threshold}")
                print(f"Last 5 FEA values: {self.machine_fea_lst[-5:]}")
            ...
            current_log.to_csv(f".\processed_data\compas\drift_checkpoints\{current_phase}_{self.name}", index= False)
...
class HiC(BRIDGET):
    ...
    def __init__(self, user_name, df_batch1, df_batch3, batch1_test, target, 
                 user_model, protected, cats, num,
                 RULE, PAST, SKEPT, GROUP, EVA, n_bins, n_var, maxc, 
                 rule_att, rule_value, hic_model,
                 drift_threshold, belief_threshold, tau_threshold, preprocessor):
        ...
        self.hic_model_name= 'ARF'
        self.machine_fea_lst= []
    ...
    def start_HiC(self, df_avv):
            ...
            for i in tqdm(range(len(self.X))):       
                ...
                if record in self.processed: #Duplicated record
                    ...
                    old_truth= int(self.processed[record]['ground_truth'])
                    #provider = self.processed[record]['provider_flag']
                    ...
                    if old_truth == user_truth:
                        self.stats[user_truth]['user']['got'] += 1

                    if old_truth == machine_prediction: 
                    ...
                    #print(f"DEBUG - Proba complete: {self.hic_model.predict_proba_one(x)}")
                    ...
                    self.machine_fea_lst.append(mach_fea)
                ...
                if i in self.fairness_records and self.GROUP:
                    DN, PP, ext = get_fairness(self.hic_model, self.protected, self.processed, self.protected_values)
                ...
                if hic_drift:
                    ...
                    dir = os.path.join("HIC_res", "compas", f"results_{self.name}")
                    ...
                    with open(os.path.join(dir,'User_'+self.name+str(self.hic_model_name)+'model.pkl'), 'wb') as file:
                            pickle.dump(self.hic_model, file)
                    with open(os.path.join(dir,'User_'+self.name+str(self.hic_model_name)+"Accuracy.txt"), "w") as f:
                            for i, value in enumerate(accuracy_score):
                                f.write(f"{i} {value}\n")
                    with open(os.path.join(dir,'User_'+self.name+str(self.hic_model_name)+"F1.txt"), "w") as f:
                            for i, value in enumerate(f1_score):
                                f.write(f"{i} {value}\n")
                    

                    with open(os.path.join(dir,'User_'+self.name+str(self.hic_model_name)+'times_LORE.txt'), 'w') as f:
                            for i, value in enumerate(times_LORE):
                                f.write(f"{i} {value}\n")

                    with open(os.path.join(dir,'User_'+self.name+str(self.hic_model_name)+"LORE_metrics.json"), "wb") as f:
                        f.write(orjson.dumps(convert_numpy(metrics_lore))) 
                    
                    with open(os.path.join(dir,'User_'+self.name+str(self.hic_model_name)+"skept.json"), "wb") as f:
                        f.write(orjson.dumps(convert_numpy(skept))) 

                    with open(os.path.join(dir, 'DRIFT_User_' + self.name + '.pkl'), 'wb') as file:
                        pickle.dump(self.hic_model, file)
                    ...
                    return df_drift_log, self.processed, self.hic_evaluation_results, accuracy_score,f1_score,equality, self.machine_fea_lst, skepticisms, self.skept_count
                

                    
                if self.EVA and i % 100 == 0:
                    human_fairness, human_acc, systemic = evaluation_human(self.processed, self.protected, self.Y,
                                                                        self.attr_list)
                
                    frank_fairness, frank_acc,frank_f1,frank_cm = evaluation_frank(self.X_test1, self.Y_test1, self.hic_model, self.protected, self.preprocessor)
                ...
                if i ==  (len(self.X)-1):
                    ...
                    with open(os.path.join(dir,'User_'+self.name+str(self.hic_model_name)+"machineFEA.txt"), "w") as f:
                            for i, value in enumerate(self.machine_fea_lst):
                                f.write(f"{i} {value}\n")
            ...
            """
                    ...
                    df_validity.to_csv(os.path.join(dir,'User_'+str(self.user)+self.name+str(self.hic_model)+"validity.csv"), index=False)

                    for x,y in zip(self.X_test1,self.Y_test1):
                        y_pred = self.hic_model.predict_one(x)
                        self.hic_acc.update(y,y_pred)    
                        self.hic_F1.update(y,y_pred)

                    with open(os.path.join(dir,'User_'+str(self.user)+self.name+str(self.hic_model)+"Acc_prequential.txt"), "w") as f:
                        f.write(str(self.hic_acc))
                    with open(os.path.join(dir,'User_'+str(self.user)+self.name+str(self.hic_model)+"F1_prequential.txt"), "w") as f:
                        f.write(str(self.hic_F1))
                    """
            ...
            return df_final_hic, self.processed, self.hic_evaluation_results, accuracy_score,f1_score,equality, self.machine_fea_lst, skepticisms, self.skept_count
...
class MiC(BRIDGET):
    ...
    def start_MiC(self, x_stream, y_stream, df_switch, r_net=None, two_step_deferral= None): ## Df_switch è l'output di HiC, ricorda di passarlo come DATA FRAME

        # old log è il dataframe prodotto da HiC, si passa come input per continuare il log
        ...
        #1. DICE structures for unrealiable instances
        times_DICE_mic= []
        sparsities_DICE_mic = []
        proximities_DICE_mic = []  # queste metriche sono tutte calcolate su dati già scalati usando self.scaler
        plausabilities_DICE_mic = []

        #2. OTHERS (PENDING)
        times_LORE_mic= []
        sparsities_LORE_mic = []
        proximities_LORE_mic = []  # queste metriche sono tutte calcolate su dati già scalati usando self.scaler
        plausabilities_LORE_mic = []



        ## Main Loop
        
        #for record, (x_rec, y_gt) in enumerate(zip(x_stream, y_stream)):
        for record, (x_rec, y_gt) in enumerate(tqdm(zip(x_stream, y_stream), total=len(x_stream))):
            ...
            if two_step_deferral: ## ANQI MAO LOGIC
                ...
                if max_conf >= self.tau:
                    ...
                    decision = int(net_output)
                    x_input_user = x_rec.squeeze().cpu().numpy()
                    user_pred= self.user_model.predict(x_input_user, y_gt, record) ## prendiamo ugualmente la predizione che avrebbe fornito l'user in questo caso
                    provider= 'M'
            ...
            mic_drift= drift_check_MiC(self.fea_mic, 
            ...
            if mic_drift:
                ...
                dir = os.path.join("MIC_res", "compas", f"{self.name}_{strat}")
                
                os.makedirs(dir, exist_ok=True)

                metrics_lore = {
                            "time_steps": times_LORE_mic,
                            "sparsity": sparsities_LORE_mic,
                            "proximity": proximities_LORE_mic,
                            "plausability":plausabilities_LORE_mic,
                            "method": "LORE",  # Aggiungi il name del metodo
                            "dataset": self.name
                            }
                
                metrics_dice = {
                            "time_steps": times_DICE_mic,
                            "sparsity": sparsities_DICE_mic,
                            "proximity": proximities_DICE_mic,
                            "plausability":plausabilities_DICE_mic,
                ...
                with open(os.path.join(dir,'User_'+self.name+str(self.mic_model_name)+"MiC_Accuracy.txt"), "w") as f:
                            f.write(f"{self.mic_acc}\n")

                with open(os.path.join(dir,'User_'+self.name+str(self.mic_model_name)+"DefNet_Accuracy.txt"), "w") as f:
                            f.write(f"{self.model_acc}\n")


                with open(os.path.join(dir,'User_'+self.name+str(self.mic_model_name)+"DefNet_Confidence.txt"), "w") as f:
                            for i, value in enumerate(mach_confidence):
                                f.write(f"{i} {value}\n")
                
                with open(os.path.join(dir,'User_'+self.name+str(self.mic_model_name)+'times_LORE_mic.txt'), 'w') as f:
                                    for i, value in enumerate(times_LORE_mic):
                                        f.write(f"{i} {value}\n")

                with open(os.path.join(dir,'User_'+self.name+str(self.mic_model_name)+"LORE_mic_metrics.json"), "wb") as f:
                                f.write(orjson.dumps(convert_numpy(metrics_lore))) 

                
                with open(os.path.join(dir,'User_'+self.name+str(self.mic_model_name)+'times_DICE_mic.txt'), 'w') as f:
                                    for i, value in enumerate(times_DICE_mic):
                                        f.write(f"{i} {value}\n")

                with open(os.path.join(dir,'User_'+self.name+str(self.mic_model_name)+"DICE_mic_metrics.json"), "wb") as f:
                                f.write(orjson.dumps(convert_numpy(metrics_dice))) 

                with open(os.path.join(dir,'User_'+self.name+str(self.mic_model_name)+"MiC_model_FEA.txt"), "w") as f:
                            for i, value in enumerate(self.fea_mic):
                                f.write(f"{i} {value}\n")


                _, df_log= get_percentage_and_df(df_switch, self.processed, self.target) 

                super().switch_phase(mic_drift, current_phase= 'MiC', current_log= df_log)

                return df_log, self.processed, self.mic_acc, self.model_acc, self.fea_mic, mach_confidence
        ...
        return df_log_final, self.processed, self.mic_acc, self.model_acc, self.fea_mic, mach_confidence
