...
class LoreTabularExplanation(TabularExplanation):
    ...
    def getExemplars(self):
        if not hasattr(self.exp, 'Z'):
            return None
        exemplars = []
        #feature_names = self.exp.dt.feature_names_in_
        for features in self.exp.Z:
            #print(features)
            exemplars.append(features)
